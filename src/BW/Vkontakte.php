...
class Vkontakte
{
     ...
     */
    public function postToPublic($publicID, $text, $fullServerPathToImage, $fullServerPathesToDocs, $tags = array())
    {
        $documents_ids = array();
        foreach ($fullServerPathesToDocs as $fullServerPathToDoc) {
            $response = $this->api('docs.getWallUploadServer', []);
            $uploadURL = $response->upload_url;
            $output = [];
            exec("curl -X POST -F 'file=@$fullServerPathToDoc' '$uploadURL'", $output);
            ...
            $documents_ids [] = 'doc'.$response[0]->owner_id.'_'.$response[0]->did;
        }
 ...
 */
        ...
        if ($fullServerPathToImage) {
            ...
            if (count($documents_ids)) {
                $attachments = $attachments . ',' . implode(',',$documents_ids);
            }
        }
        else {
            $attachments = implode(',',$documents_ids);
        }
        $response = $this->api('wall.post',
            [
                ...
                'from_group' => 1,
                'message' => "$text",
                'attachments' => $attachments, // uploaded image and files are passed as attachment
            ]);
